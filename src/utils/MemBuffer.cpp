...
// allocation methods of heap objects in running script, which will be freed all together after script execution finishes even if user couldn't handle freeing the objects in their script.
...
void MemBuffer::freeAllHeapEntries() {
    ...
    while (block) {
        if (block->itemCount > 0) {
            ...
            while (true) {
                MemBufferBlock* blockRef = *(MemBufferBlock**)((char*)block->chunk + offset + HEADER - sizeof(MemBufferBlock*));
                if (blockRef == block) { // found an item that belongs to this block
                    HeapEntry* heapEntry = (HeapEntry*)((char*)block->chunk + offset + HEADER);
                    if (!heapEntry->freed) {
                        free(heapEntry->ptr);
                        heapEntry->freed = true;
                    }
                    offset += HEADER + sizeof(HeapEntry);
                }
                else {
