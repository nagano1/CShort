...
//
    ...
    /*
	* Before editing this file, it is required to modify indentation to follow CShort's indent rule
	*/
	...
	void DocumentUtils::checkIndentSyntaxErrors(DocumentStruct* doc)
	{
		...
		while (line) {
			...
			if (line->depth > 0 && node->vtable != VTables::LineBreakVTable) {
				int nodeIndent = node->precedingSpaceCount;
				bool ok = false;
	...
	*/
	static void indentFormatLine(CodeLine* line, bool justKeepRule) {
		...
		if (firstElement) {
			...
			// modify indent
			if (justKeepRule) {
				if (firstElement->precedingSpaceCount >= line->depth * baseIndent) {
					return;
				}
			}
			int textLen = static_cast<int>(line->depth * baseIndent);
			firstElement->precedingSpaceCount = textLen;
		}
	}
